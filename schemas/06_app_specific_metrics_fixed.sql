-- =====================================================
-- APP-SPECIFIC METRICS SUPPORT - Fixed Version
-- =====================================================
-- 
-- Purpose: Enable proper KPI tracking for onboarding and offboarding
-- Fixed: Removed subquery constraint that's not supported in PostgreSQL
...
-- Note: Removed the CHECK constraint with subquery as it's not supported
-- Data integrity will be enforced at the application level instead
...
-- Trigger on task changes (separate triggers for each operation due to WHEN clause limitations)
DROP TRIGGER IF EXISTS trigger_update_enrollment_counts ON api.tasks;

-- Trigger for INSERT and UPDATE
CREATE TRIGGER trigger_update_enrollment_counts_insert_update
...
-- Trigger for DELETE  
CREATE TRIGGER trigger_update_enrollment_counts_delete
  AFTER DELETE ON api.tasks
  FOR EACH ROW
  WHEN (OLD.app_id IS NOT NULL)
  EXECUTE FUNCTION update_enrollment_task_counts();
...
CREATE OR REPLACE FUNCTION api.get_client_onboarding_count(client_uuid UUID)
RETURNS INTEGER AS $$
BEGIN
  RETURN (
    SELECT COUNT(*)
...
CREATE OR REPLACE FUNCTION api.get_client_offboarding_count(client_uuid UUID)
RETURNS INTEGER AS $$
BEGIN
  RETURN (
    SELECT COUNT(*)
...
-- =====================================================
-- VALIDATION FUNCTION (replaces CHECK constraint)
-- =====================================================

-- Function to validate task-application relationship
-- This replaces the CHECK constraint that caused the error
CREATE OR REPLACE FUNCTION api.validate_task_app_relationship(
  task_employee_id UUID,
  task_app_id UUID
) RETURNS BOOLEAN AS $$
BEGIN
  -- If app_id is NULL, it's valid
  IF task_app_id IS NULL THEN
    RETURN TRUE;
  END IF;
  
  -- Check if the task's employee and application belong to the same client
  RETURN EXISTS (
    SELECT 1 
    FROM api.client_applications ca 
    JOIN api.employees e ON e.client_id = ca.client_id 
    WHERE ca.id = task_app_id AND e.id = task_employee_id
  );
END;
$$ LANGUAGE plpgsql;
...
COMMENT ON FUNCTION api.get_client_onboarding_count IS 'Returns count of employees in active onboarding for specified client';
COMMENT ON FUNCTION api.get_client_offboarding_count IS 'Returns count of employees in active offboarding for specified client';
COMMENT ON FUNCTION api.validate_task_app_relationship IS 'Validates that task employee and application belong to same client (replaces CHECK constraint)';

-- =====================================================
-- FINAL NOTES
-- =====================================================

-- Note: The constraint that was causing the error has been removed.
-- Data integrity for task-application relationships should be enforced
-- at the application level using the validate_task_app_relationship function.
-- 
-- Example usage in application code:
-- SELECT api.validate_task_app_relationship(employee_id, app_id) 
-- before inserting/updating tasks.