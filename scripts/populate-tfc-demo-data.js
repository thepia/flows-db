...
function calculateTFCPricing(creditAmount) {
  ...
  return {
    ...
    discountAmount: creditAmount * (150.00 - pricePerCredit)
  };
}
...
async function generatePurchaseHistory(clientId, config, primaryContact, billingContact) {
  ...
  const initialAmount = config.purchasePattern === 'annual_with_topups' ? config.annualPurchase : 500;
  ...
  while (currentDate < new Date()) {
    if (config.purchasePattern === 'quarterly') {
      ...
      if (currentDate.getMonth() % 3 === 0) {
        const amount = Math.floor(Math.random() * (config.bulkSize.max - config.bulkSize.min + 1)) + config.bulkSize.min;
      ...
      if (currentDate.getMonth() === startDate.getMonth() && currentDate.getDate() === startDate.getDate()) {
        ...
        if (payment) {
          ...
          purchases.push({ date: new Date(currentDate), amount: config.annualPurchase, type: 'purchase' });
        }
      }
      
      // Quarterly top-ups
      if ((currentDate.getMonth() + 1) % 3 === 0) {
        const amount = Math.floor(Math.random() * (config.quarterlyTopup.max - config.quarterlyTopup.min + 1)) + config.quarterlyTopup.min;
        const pricing = calculateTFCPricing(amount);
...
async function generateUsageHistory(clientId, config, purchases) {
  ...
  while (currentDate < new Date()) {
    ...
    const monthlyUsage = generateMonthlyUsagePattern(config.monthlyUsage.min, config.monthlyUsage.max, month);
    
    // Distribute usage across the month
    for (let i = 0; i < monthlyUsage; i++) {
      const usageDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), Math.floor(Math.random() * 28) + 1);
      ...
      const { data: transaction } = await supabase
        ...
        .insert({
          ...
          price_per_credit: 150.00, // Rate at time of usage
          total_amount: -150.00,
          currency: config.preferredCurrency,
      ...
      if (transaction) {
        ...
        await supabase
          ...
          .insert({
            ...
            seniority_level: ['junior', 'mid', 'senior', 'lead', 'executive'][Math.floor(Math.random() * 5)],
...
async function populateTFCDataForCompany(clientCode) {
  ...
  try {
    ...
    const purchases = await generatePurchaseHistory(client.id, config, primaryContact, billingContact);
    ...
    if (balance) {
      ...
      console.log(`   - Average Price: ${config.preferredCurrency} ${balance.average_credit_price} per TFC`);
    }
