...
function validateData(analysis) {
  ...
  // Total people count
  const totalValid = analysis.total >= EXPECTED.totalPeople.min && analysis.total <= EXPECTED.totalPeople.max;
  results.push({
  ...
  // Employee count
  const employeeValid = analysis.employees >= EXPECTED.employees.min && analysis.employees <= EXPECTED.employees.max;
  results.push({
    test: 'Employee Count',
  ...
  // Associate count
  const associateValid = analysis.associates >= EXPECTED.associates.min && analysis.associates <= EXPECTED.associates.max;
  results.push({
  ...
  // Active employees
  const activeValid = analysis.statusCounts.active >= EXPECTED.activeEmployees.min && analysis.statusCounts.active <= EXPECTED.activeEmployees.max;
  results.push({
  ...
  // Former employees
  const formerValid = analysis.statusCounts.former >= EXPECTED.formerEmployees.min && analysis.statusCounts.former <= EXPECTED.formerEmployees.max;
  results.push({
  ...
  // Future employees
  const futureValid = analysis.statusCounts.future >= EXPECTED.futureEmployees.min && analysis.statusCounts.future <= EXPECTED.futureEmployees.max;
  results.push({
  ...
  const domainValid = analysis.emailDomains.length === 1 && analysis.emailDomains[0] === EXPECTED.companyDomain;
  results.push({
  ...
  results.forEach(result => {
    ...
    console.log(`${status} ${chalk.cyan(result.test.padEnd(25))}: ${chalk.white(result.actual)} ${chalk.gray(`(expected: ${result.expected})`)}`);
  });
...
function displayAnalysis(analysis) {
  ...
  console.log(`   Active: ${chalk.green(analysis.statusCounts.active)} (${Math.round(analysis.statusCounts.active / analysis.employees * 100)}%)`);
  console.log(`   Former: ${chalk.yellow(analysis.statusCounts.former)} (${Math.round(analysis.statusCounts.former / analysis.employees * 100)}%)`);
  console.log(`   Future: ${chalk.blue(analysis.statusCounts.future)} (${Math.round(analysis.statusCounts.future / analysis.employees * 100)}%)`);
  ...
  console.log(`   Departments: ${chalk.white(analysis.departments.length)} (${analysis.departments.slice(0, 3).join(', ')}${analysis.departments.length > 3 ? '...' : ''})`);
  console.log(`   Associate Types: ${chalk.white(analysis.associateTypes.length)} (${analysis.associateTypes.join(', ')})`);
  ...
  if (analysis.samplePeople.length > 0) {
    ...
    analysis.samplePeople.forEach(person => {
      ...
      console.log(`   ${chalk.white(person.person_code)}: ${person.name} - ${person.department} (${status})`);
    });
    console.log('');
  }
}
