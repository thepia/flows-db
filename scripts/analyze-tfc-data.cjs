...
async function analyzeAvailableTFCData() {
  ...
  if (balance) {
    ...
    console.log(`  ✓ Thresholds: Low ${balance.low_balance_threshold}, Critical ${balance.critical_balance_threshold}`);
  }
  ...
  if (payments) {
    payments.forEach(p => {
      console.log(`  ✓ ${p.payment_amount} ${p.currency} - ${p.payment_method} (${p.payment_status})`);
    });
  }
