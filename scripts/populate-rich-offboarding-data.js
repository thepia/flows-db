...
async function populateRichOffboardingData() {
  ...
  try {
    ...
    const departmentNotes = {
      'Engineering': ['Code review handover needed', 'Access keys to revoke', 'Technical documentation transfer'],
      'Product': ['Project roadmap handover', 'Stakeholder communication needed', 'User research data transfer'],
      'Design': ['Design system maintenance', 'Asset library handover', 'Brand guideline review'],
      'Marketing': ['Campaign handover required', 'Social media access removal', 'Content calendar transfer'],
      ...
      'Finance': ['Budget responsibility transfer', 'Expense approvals setup', 'Financial system access']
    };
    ...
    for (let i = 0; i < people.length; i++) {
      ...
      const completion = Math.floor(
        Math.random() * (statusInfo.completion[1] - statusInfo.completion[0]) + statusInfo.completion[0]
      );
      
      // Calculate dates based on priority and status
      const daysToComplete = Math.floor(
        Math.random() * (priorityInfo.daysToComplete[1] - priorityInfo.daysToComplete[0]) + priorityInfo.daysToComplete[0]
      );
      ...
      const overdueTasks = statusInfo.status === 'active' && Math.random() > 0.7 ? Math.floor(Math.random() * 2) : 0;
      ...
      const estimatedHours = Math.floor(Math.random() * (hourRange[1] - hourRange[0]) + hourRange[0]);
      ...
      if (['active', 'completed'].includes(statusInfo.status)) {
        ...
        if (Math.random() > 0.3) { // 70% get HR approval
        ...
        if (['Engineering', 'Product'].includes(person.department) && Math.random() > 0.5) {
          securityApproved = new Date((hrApproved || managerApproved).getTime() + Math.random() * 24 * 60 * 60 * 1000);
        }
      }
    ...
    for (let i = 0; i < processes.length; i += batchSize) {
      ...
      try {
        ...
        if (processError) {
          ...
          console.log(`✅ Batch ${Math.floor(i/batchSize) + 1}: Created ${createdProcesses.length} processes`);
    ...
    console.log('   Department-specific notes for', Object.keys(departmentNotes).length, 'departments');
