...
function displayDeploymentInstructions() {
  ...
  console.log(chalk.white('To deploy the new task-oriented offboarding system, please follow these steps:\n'));
  ...
  console.log(chalk.white('   → Check that new tables exist: offboarding_templates, offboarding_task_templates, etc.'));
  console.log(chalk.white('   → Run: npm run health-check'));
  console.log(chalk.white('   → Populate demo data: node scripts/create-offboarding-templates-demo.js\n'));
...
async function deployTaskOffboardingSchema() {
    ...
    const { proceed } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'proceed',
        message: 'Schema appears to already exist. This will update/replace existing tables. Continue?',
        default: false
      }
    ]);
  ...
  console.log(chalk.white('  3. Create demo data: node scripts/create-offboarding-templates-demo.js'));
