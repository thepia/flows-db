...
test.describe('Filter Results Count', () => {
  test('should show correct filtered count in "Showing X of Y people" text', async ({ page }) => {
    ...
    const totalPeopleCard = await page
      .locator('dt:has-text("Total People")')
      .locator('..')
      .locator('dd')
      .textContent();
    const totalPeople = Number.parseInt(totalPeopleCard || '0');
    ...
    const initialShowingText = await page
      .locator('text=/Showing \\d+ of \\d+ people/')
      .first()
      .textContent();
    ...
    const initialTotal = initialMatch ? Number.parseInt(initialMatch[1]) : 0;
    ...
    const filteredShowingText = await page
      .locator('text=/Showing \\d+ of \\d+ people/')
      .first()
      .textContent();
    ...
    const filteredTotal = filteredMatch ? Number.parseInt(filteredMatch[1]) : 0;
    console.log(`📈 Filtered "of" count: ${filteredTotal}`);

    // Verify the stats card still shows the full total (unchanged)
    const totalPeopleCardAfter = await page
      .locator('dt:has-text("Total People")')
      .locator('..')
      .locator('dd')
      .textContent();
    const totalPeopleAfter = Number.parseInt(totalPeopleCardAfter || '0');
    ...
    console.log(
      `✅ "Showing X of Y" correctly shows filtered total: ${filteredTotal} < ${totalPeople}`
    );
    ...
    const clearedShowingText = await page
      .locator('text=/Showing \\d+ of \\d+ people/')
      .first()
      .textContent();
    const clearedMatch = clearedShowingText?.match(/Showing \d+ of (\d+) people/);
    const clearedTotal = clearedMatch ? Number.parseInt(clearedMatch[1]) : 0;
  ...
  test('should show correct count when searching', async ({ page }) => {
    ...
    const searchTotal = searchMatch ? Number.parseInt(searchMatch[1]) : 0;
