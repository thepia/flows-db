...
test.describe('Data Consistency Test Suite', () => {
  ...
  test.describe('PersonEnrollment Data Consistency', () => {
    ...
    test('should handle mixed employee and associate data correctly', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(personCount, 10); i++) {
        ...
        const hasEnrollment =
          (await page.locator('[data-testid="enrollment-progress"]').count()) > 0;
        if (hasEnrollment) {
    ...
    test('should handle enrollment data lookup correctly (personId vs employeeId fix)', async ({
      page,
    }) => {
      ...
      for (let i = 0; i < testCount; i++) {
        ...
        const enrollmentErrors = consoleErrors.filter(
          (error) =>
            error.includes('employeeId') ||
            error.includes('personId') ||
        ...
        if ((await enrollmentSection.count()) > 0) {
          ...
          if ((await percentageElement.count()) > 0) {
            ...
            const percentage = Number.parseInt(percentageText.replace('%', ''));
      ...
      const criticalEnrollmentErrors = consoleErrors.filter(
        (error) =>
  ...
  test.describe('Cross-Reference Data Validation', () => {
    test('should maintain data consistency between People and Employees views', async ({
      page,
    }) => {
          ...
          for (let i = 0; i < checkCount; i++) {
            ...
            if ((await nameElement.count()) > 0 && (await emailElement.count()) > 0) {
              ...
              const matchingPerson = peopleData.find(
                (person) => person.email === employeeEmail || person.name === employeeName
              );
    ...
    test('should handle client switching without data corruption', async ({ page }) => {
      ...
      // Get initial client
      const currentClient = await page
        .textContent('[data-testid="current-client-name"]')
        .catch(() => null);
      ...
      if ((await clientSwitcher.count()) > 0) {
        ...
        if ((await clientOptions.count()) > 1) {
          ...
          await clientSwitcher.click();
          if (currentClient) {
            const originalClientOption = page.locator(
              `[data-testid="client-option-${currentClient}"]`
            );
  ...
  test.describe('Data Loading State Consistency', () => {
    test('should show appropriate loading states during data fetch', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="view-people"]', {
        state: 'visible',
        timeout: 10000,
      });
