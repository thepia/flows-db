...
test.describe('Client Loading Priority (Regression)', () => {
  test('should load Hygge & Hvidløg as default client, not Nets A/S (Demo)', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="client-name"], .client-name, h1, h2', { timeout: 10000 });
    ...
    for (const selector of clientNameSelectors) {
      try {
        ...
        for (const element of elements) {
          ...
          if (text && (text.includes('Hygge') || text.includes('Hvidløg') || text.includes('Nets'))) {
  ...
  test('should show Hygge & Hvidløg employees in people table', async ({ page }) => {
    ...
    await page.waitForSelector('table, [data-testid="people-table"], .employee-list', { timeout: 10000 });
