  ...
  test.describe('Settings Integration', () => {
    ...
    test('should handle client switching from settings', async ({ page }) => {
      ...
      if (clientSwitcher) {
        ...
        const currentClientName = await page.locator('[data-testid="current-client-name"]').textContent().catch(() => null);
        ...
        if (await clientOptions.count() > 1) {
          ...
          const newClientName = await page.locator('[data-testid="current-client-name"]').textContent().catch(() => null);
