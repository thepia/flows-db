...
import { NavigationContract, NavigationPaths, NavigationActions } from './contracts/navigation.contract.js';
...
test.describe('Navigation Test Suite', () => {
  ...
  test.describe('Main Tab Navigation', () => {
    ...
    test('should navigate between tabs', async ({ page }) => {
      ...
      const appTabs = await page.locator('[data-testid^="tab-"]:not([data-testid="tab-people"])').all();
      if (appTabs.length > 0) {
    ...
    test('should maintain tab state when navigating within a tab', async ({ page }) => {
      ...
      const offboardingTab = await page.locator('[data-testid^="tab-"]').filter({ hasText: 'Offboarding' }).first();
  ...
  test.describe('State Persistence', () => {
    ...
    test('should maintain selected process when switching between processes and tasks', async ({ page }) => {
      // Navigate to offboarding tab
      const offboardingTab = await page.locator('[data-testid^="tab-"]').filter({ hasText: 'Offboarding' }).first();
      if (await offboardingTab.count() === 0) {
        test.skip();
  ...
  test.describe('Error Handling', () => {
    test('should handle navigation when data is loading', async ({ page }) => {
      ...
      await expect(loadingIndicator).toBeInViewport({ timeout: 1000 }).catch(() => {
...
test.describe('Navigation Performance', () => {
  test('should complete navigation within acceptable time', async ({ page }) => {
    ...
    const appTab = await page.locator('[data-testid^="tab-"]:not([data-testid="tab-people"])').first();
