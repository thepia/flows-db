...
test.describe('Debug Application Content', () => {
  test('should debug what happens when clicking application tabs', async ({ page }) => {
    ...
    for (let i = 0; i < appTabs.length; i++) {
      ...
      const metricsVisible = await page.locator('text="Active Invitations"').isVisible().catch(() => false);
      const overviewBtnVisible = await page.locator('[data-testid="offboarding-view-overview"]').isVisible().catch(() => false);
      const templatesBtnVisible = await page.locator('[data-testid="offboarding-view-templates"]').isVisible().catch(() => false);
