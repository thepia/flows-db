...
test.describe('Offboarding App Tab', () => {
  ...
  test.describe('Navigation', () => {
    test('should navigate to offboarding tab', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="tab-offboarding"]')).toHaveAttribute(
        'data-active',
        'true'
      );
    ...
    test('should show overview as default view', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="offboarding-view-overview"]')).toHaveAttribute(
        'data-active',
        'true'
      );
      await expect(page.locator('[data-testid="view-overview"]')).toBeVisible();
    ...
    test('should navigate between offboarding views', async ({ page }) => {
      ...
      for (const view of views) {
        ...
        await expect(page.locator(`[data-testid="offboarding-view-${view}"]`)).toHaveAttribute(
          'data-active',
          'true'
        );
      }
  ...
  test.describe('Overview Dashboard', () => {
    test('should display all metric cards', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="metric-card-active"] .metric-value')).toContainText(
        '3'
      );
      await expect(
        page.locator('[data-testid="metric-card-ending-soon"] .metric-value')
      ).toContainText('1');
      await expect(
        page.locator('[data-testid="metric-card-completed"] .metric-value')
      ).toContainText('5');
      await expect(
        page.locator('[data-testid="metric-card-attention"] .metric-value')
      ).toContainText('2');
    });
    ...
    test('should navigate to processes when clicking metric cards', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="offboarding-view-processes"]')).toHaveAttribute(
        'data-active',
        'true'
      );
    });
    ...
    test('should navigate to templates when clicking create offboarding', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="offboarding-view-templates"]')).toHaveAttribute(
        'data-active',
        'true'
      );
    });
  ...
  test.describe('Template Management', () => {
    ...
    test('should display template details', async ({ page }) => {
      ...
      await expect(template.locator('.template-name')).toContainText(
  ...
  test.describe('Process Management', () => {
    ...
    test('should select process and navigate to tasks', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="view-tasks"]')).toBeVisible();
      await expect(page.locator('[data-testid="offboarding-view-tasks"]')).toHaveAttribute(
        'data-active',
        'true'
      );
    });
  ...
  test.describe('Task Management', () => {
    ...
    test('should display tasks in correct columns', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="task-column-completed"] [data-testid="task-task-001"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-column-in_progress"] [data-testid="task-task-002"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-column-pending"] [data-testid="task-task-003"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-column-blocked"] [data-testid="task-task-004"]')
      ).toBeVisible();
    });
    ...
    test('should display blocked task reason', async ({ page }) => {
      ...
      await expect(blockedTask.locator('.blocked-reason')).toContainText(
  ...
  test.describe('Accessibility', () => {
    test('should support keyboard navigation', async ({ page }) => {
      ...
      await expect(page.locator(':focus')).toHaveAttribute(
        'data-testid',
        'offboarding-view-overview'
      );

      await page.keyboard.press('Tab');
      await expect(page.locator(':focus')).toHaveAttribute(
        'data-testid',
        'offboarding-view-templates'
      );
    ...
    test('should have proper ARIA labels', async ({ page }) => {
      ...
      // Check button labels
      await expect(page.locator('[data-testid="create-offboarding-button"]')).toHaveAttribute(
        'aria-label'
      );
