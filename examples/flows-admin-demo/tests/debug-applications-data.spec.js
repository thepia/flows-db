...
test.describe('Debug Applications Data', () => {
  test('should examine applications data in browser', async ({ page }) => {
    ...
    const applicationsData = await page.evaluate(() => {
      ...
      const appButtons = Array.from(document.querySelectorAll('[data-testid^="tab-"]:not([data-testid="tab-people"]):not([data-testid="tab-processes"]):not([data-testid="tab-account"])'));
    ...
    for (let i = 0; i < appTabs.length; i++) {
      ...
      const hasSelectedApp = await page.evaluate(() => {
        ...
        const hasOffboardingView = !!document.querySelector('[data-testid="offboarding-view-overview"]');
        const hasMetricsView = Array.from(document.querySelectorAll('*')).some(el => el.textContent?.includes('Active Invitations'));
        ...
        return {
          ...
          hasMetricsView,  
          hasAppHeader,
          mainContentHTML: document.querySelector('main')?.innerHTML?.substring(0, 500) || 'No main content'
        };
      });
