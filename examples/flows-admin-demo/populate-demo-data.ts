...
async function populateDemoData() {
  ...
  try {
    ...
    const hyggeClient = await supabase.from('clients').select('id').eq('client_code', 'hygge-hvidlog').single();
    if (hyggeClient.data) {
    ...
    const meridianClient = await supabase.from('clients').select('id').eq('client_code', 'meridian-brands').single();
    ...
    const hyggeVerifyClient = await supabase.from('clients').select('id').eq('client_code', 'hygge-hvidlog').single();
    const meridianVerifyClient = await supabase.from('clients').select('id').eq('client_code', 'meridian-brands').single();
    
    const { count: hyggeEmployees } = await supabase.from('people').select('*', { count: 'exact', head: true }).eq('client_id', hyggeVerifyClient.data.id);
    const { count: meridianEmployees } = await supabase.from('people').select('*', { count: 'exact', head: true }).eq('client_id', meridianVerifyClient.data.id);
