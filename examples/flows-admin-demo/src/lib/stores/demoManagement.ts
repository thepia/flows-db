...
function createDemoManagementStore() {
	...
	return {
		 ...
		 */
		createCompany: async (companyData: Omit<DemoCompany, 'id' | 'lastGenerated' | 'generationStatus'>) => {
		...
		generateDemoData: async (config: DemoGenerationConfig) => {
			...
			update(state => ({
				...
				companies: state.companies.map(company =>
					company.id === config.companyId 
						? { ...company, generationStatus: 'generating' }
						: company
				)
			}));
			...
			try {
				...
				update(state => ({
					...
					companies: state.companies.map(company =>
						company.id === config.companyId 
							? { ...company, generationStatus: 'error' }
							: company
					),
					actions: state.actions.map(action =>
						action.id === actionId
		...
		clearOldActions: () => {
			...
			update(state => ({
				...
				actions: state.actions.filter(action => 
					action.startedAt > oneDayAgo || action.status === 'running'
				)
			}));
		},
