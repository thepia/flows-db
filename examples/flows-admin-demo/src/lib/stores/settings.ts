...
import type { DemoSettings, SettingsState, BrandingConfig, Client } from '$lib/types';
...
function createSettingsStore() {
	...
	return {
		 ...
		 */
		selectBranding: (brandingId: string) => {
			update(state => {
				const newSettings = {
					...state.settings,
		...
		addBranding: (branding: BrandingConfig) => {
			update(state => {
				const existingIndex = state.settings.availableBrandings.findIndex(b => b.id === branding.id);
...
export function getAvailableClients(allClients: Client[], settings: DemoSettings): Client[] {
	...
	// Filter to only demo/test clients
