...
export async function getDemoMetrics(companyCode?: string): Promise<{
  ...
  // Get metrics
  const [companiesResult, employeesResult, invitationsResult, documentsResult, tasksResult] = await Promise.all([
  ...
  const activeProcesses = invitations.filter(inv => ['pending', 'sent'].includes(inv.status)).length;
